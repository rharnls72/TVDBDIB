<template>
    <b-nav justified class="myfooter pt-1">
      ...
      <b-nav-item @click="changeState(2), moveTab('IndexSearch')">
        <b-icon-search v-if="this.$store.state.tabState == 2" class="text-dark"></b-icon-search>
        <b-icon-search v-else class="text-secondary"></b-icon-search>
      </b-nav-item>
      <!-- 3. 큐레이션(default) -->
      <b-nav-item @click="changeState(3), moveTab('IndexCuration')">
        <b-icon-house-door-fill v-if="this.$store.state.tabState == 3" class="text-dark"></b-icon-house-door-fill>
        <b-icon-house-door v-else class="text-secondary"></b-icon-house-door>
      </b-nav-item>
      <!-- 4. 알림 -->
      <b-nav-item @click="changeState(4), moveTab('Alert')">
        <b-icon-bell-fill v-if="this.$store.state.tabState == 4" class="text-dark"></b-icon-bell-fill>
        <b-icon-bell v-else class="text-secondary"></b-icon-bell>
      </b-nav-item>
      <!-- 5. 내 프로필 -->
      <b-nav-item @click="changeState(5), moveTab('MyPage')">
        <b-icon-person-circle v-if="this.$store.state.tabState == 5" class="text-dark"></b-icon-person-circle>
